...
sealed class ASRProviderSetting {
    ...
    /**
     * Legacy local System STT (Android SpeechRecognizer).
     *
     * Kept solely for backward-compatibility deserialization of old settings JSON.
     * Not shown in the UI, not registered in [Types], and produces no controller.
     * [normalizeSttSettings] filters these out on load.
     */
    @Serializable
    @SerialName("system")
    ...
    /**
     * OpenAI-compatible REST STT provider.
     *
     * Works with any provider that exposes the OpenAI /audio/transcriptions endpoint:
     * OpenAI, Groq, Regolo AI, DeepInfra, Together AI, Fireworks AI, xAI, and custom
     * OpenAI-compatible endpoints. Audio is recorded in segments, converted to WAV,
     * and POSTed per segment; the returned text is concatenated.
     *
     * OpenRouter is auto-detected via [baseUrl] and uses a JSON + base64 body
     * ({model, input_audio:{data, format}, language}) instead of multipart form.
     *
     * Models can be discovered dynamically via GET {baseUrl}/models (OpenRouter:
     * GET {baseUrl}/models?output_modalities=transcription).
     */
    @Serializable
    @SerialName("openai_compatible")
    data class OpenAICompatible(
        override val id: Uuid = Uuid.random(),
        override val name: String = "OpenAI-compatible STT",
        override val customIconUri: String? = null,
        override val enabled: Boolean = true,
        val apiKey: String = "",
        ...
        val sampleRate: Int = 16000,
        // 每多少秒自动 flush 一次当前缓冲区 (上传识别)。0 = 仅 stop() 时整体上传
        val segmentDurationSec: Int = 30,
    ) : ASRProviderSetting() {
        override fun copyProvider(
            id: Uuid,
        ...
        /** Whether this baseUrl points to OpenRouter (uses JSON/base64 body format). */
        val isOpenRouter: Boolean
            get() = baseUrl.contains("openrouter.ai", ignoreCase = true)
    }

    companion object {
        val Types by lazy {
            listOf(
                OpenAICompatible::class,
